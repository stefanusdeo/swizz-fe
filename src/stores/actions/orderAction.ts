...
export const addCartList = (newProduct: ICart, cartList: ICart[]) => async (dispatch: any) => {
    try {
        let newCart: ICart[] = cartList.map(obj => ({ ...obj }));
        
        if (newCart.length !== 0) {
            let isAnyProduct = cartList.findIndex(x => x.product?.id === newProduct.product?.id)
            if (isAnyProduct >= 0) {
                if (newCart[isAnyProduct].isCustom == false) {
                    newCart[isAnyProduct].qty = (newCart[isAnyProduct].qty ?? 0) + (newProduct.qty ?? 0)
                }
                else {
...
export const submitCheckout = (body: ICheckoutOrder) => async (dispatch: any) => {
    try {
