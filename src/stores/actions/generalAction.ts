...
export const setLanguage = (lang: string) => async (dispatch: any) => {
  try {
...
export const setCountry = (country: string) => async (dispatch: any) => {
  try {
...
export const getAboutUs = (cat: string) => async (dispatch: any) => {
  try {
...
export const getTermCond = (cat: string) => async (dispatch: any) => {
  try {
...
export const getBanner = (cat: string) => async (dispatch: any) => {
  try {
    ...
    if (response && response.status === 200) {
      ...
      let bottomBanner = data.filter(
        (x) => x.type === 'Bottom' || x.type === 'bottom'
      );
      const respSetLang: ICallbackGeneral = {
        code: 200,
